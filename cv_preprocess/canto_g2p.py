...
if len(sys.argv) != 3:
    print("Usage: python canto_g2p.py <input_wordlist.txt> <output_dictionary.txt>")
    sys.exit(1)
...
pron = {
    ...
    for l in open("yue.tsv", "r", encoding="utf-8").readlines()
}

with open(output_path, "w", encoding="utf-8") as output:  # "lexicon.txt"

    rows = []
    with open(input_path, "r", encoding="utf-8") as f:  # "words.txt"
        for line in tqdm(f):
            w = line.strip()
            word_pron = ""
            if w in pron:
